...
public class RelatorioDAO {
    ...
    public static List<Relatorio> getProdutosMaisVendidos(String dataInicio, String dataFim, int filial_id) {
        List<Relatorio> relatorios = new ArrayList<>();
        String query = "SELECT c.produto_id, c.descricao, COUNT(p.fk_produto_id) AS QtdCompra, SUM(p.valor_unitario) AS Total"+
                       "FROM Pedido p"+ 
                       "INNER JOIN Produto c ON c.produto_id = p.fk_produto_id"+ 
                       "WHERE p.data BETWEEN ? AND ? AND p.fk_filial_id = ?"+
                       "GROUP BY c.produto_id, c.descricao"+
                       "ORDER BY Total DESC";
        Connection con;
        try {
            con = Conexao.getConexao();
...
public static List<Relatorio> getFiliais() {
        List<Relatorio> relatorios = new ArrayList<>();
        String query = "SELECT filial_id, endereco" +
                       "FROM Filial ";
        Connection con;
        try {
            con = Conexao.getConexao();
