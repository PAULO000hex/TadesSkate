...
public class RelatorioDAO {
    ...
    public static List<Relatorio> getProdutosMaisVendidos(int filial_id) {
        List<Relatorio> relatorios = new ArrayList<>();
        String query = "SELECT c.produto_id, c.descricao, COUNT(p.fk_produto_id) AS QtdCompra, SUM(p.valor_unitario) AS Total FROM Pedido p INNER JOIN Venda v ON v.venda_id = p.fk_venda_id INNER JOIN Produto c ON c.produto_id = p.fk_produto_id WHERE p.fk_filial_id = ? GROUP BY c.produto_id, c.descricao ORDER BY Total DESC";
        Connection con;
        try {
            con = Conexao.getConexao();
            System.out.println(con);
